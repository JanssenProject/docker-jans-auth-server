...
def jks_created():
    manager.secret.to_file("auth-server_jks_base64", "/etc/certs/auth-server-keys.jks", decode=True, binary_mode=True)
    return True
...
def jwks_created():
    with open("/etc/certs/auth-server-keys.json", "w") as f:
        f.write(base64.b64decode(
            manager.secret.get("auth-server_openid_key_base64")
        ).decode())
    return True


def should_sync_jks():
    last_rotation = manager.config.get("auth-server_key_rotated_at")
    ...
    try:
        mtime = int(os.path.getmtime(manager.config.get("auth-server_openid_jks_fn")))
    except OSError:
        mtime = 0
